...
import { Shield, Search, Clock, User, Download } from "lucide-react";
...
export default function ActivityLogsClient({ initialLogs, isAdmin }: ActivityLogsClientProps) {
  const [logs, setLogs] = useState<any[]>(initialLogs);
  const [search, setSearch] = useState("");
  const [actionFilter, setActionFilter] = useState("");
  const [loading, setLoading] = useState(false);

  const loadLogs = async (searchVal: string, actionVal: string) => {
    setLoading(true);
    try {
