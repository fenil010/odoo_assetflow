...
import { updateProfileData, updateUserPassword } from "@/actions/profile";
...
export default function SettingsPageClient({ initialData }: SettingsPageClientProps) {
  const [profile] = useState<any>(initialData);
  ...
  // General profile state (pre-populated from server)
  ...
  const [theme, setTheme] = useState(initialData?.appearanceTheme || "light");
  const [emailAlerts, setEmailAlerts] = useState(initialData?.emailNotificationsEnabled ?? true);
